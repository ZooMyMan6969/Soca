...
local menu_elements_sorc_base =
{
    ...
    ignore_functions      = checkbox:new(false, get_hash(my_utility.plugin_label .. "ignore_functions_teleport")),
...
local function menu()
    ...
    if menu_elements_sorc_base.tree_tab:push("Teleport") then
        ...
        menu_elements_sorc_base.ignore_functions:render("Ignore Functions, just spam", "");
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    local ignore_functions = menu_elements_sorc_base.ignore_functions:get();
    ...
    if ignore_functions then
        ...
        if area_data.main_target:is_enemy() then
            cast_spell.position(spell_id_tp, area_data.main_target:get_position(), 0.3);
            local current_time = get_time_since_inject();
            next_time_allowed_cast = current_time + 0.4;
            console.print("Sorcerer Plugin, Casted Tp (Ignoring Functions)");
            return true;
        end
    else
        -- Original logic
